...
(function() {
    ...
    document.write('<link rel="stylesheet" type="text/css" href="' + path + '/ux/grid/css/GridFilters.css"/>');
    document.write('<link rel="stylesheet" type="text/css" href="' + path + '/ux/grid/css/RangeMenu.css"/>');

    document.write('<script type="text/javascript" src="' + path + '/ext-all' + (rtl ? '-rtl' : '') + '.js"></script>');
    
    if (local != undefined && local != '') {
        document.write('<script type="text/javascript" src="' + path + '/locale/ext-lang-' + local + '.js"></script>');
    }
    ...
    if (custom) {
        customPath = (repoDevMode ? path + '/..' : path) +
            '/ext-theme-custom' +
            (repoDevMode ? '-dev' : '') + '.js';
        ...
        if (repoDevMode &&  window.ActiveXObject) {
            Ext = {
                _beforereadyhandler: function() {
                    Ext.Loader.loadScript({ url: customPath });
                }
            };
        } else {
            document.write('<script type="text/javascript" src="' + customPath + '" defer></script>');
        }
    }
})();